...
(HELPPP!!!) For many regions
```{r}
hansen
```

```{r}
jc_sf <- st_as_sf(jc, crs=3414, coords=c('X','Y'))
#jc_sf <- st_as_sf(jc, crs=3414, coords=c('LONGITUDE','LATITUDE'))
#hdb_sf <- st_as_sf(hdb, crs=3414, coords=c('LONGITUDE','LATITUDE'))
```
```{r}
...
hansen_sf <- st_as_sf(hansen, crs=3414)
...
ggplot(data=hansen_mpsz, 
       aes(y = hansen_mpsz$durationHansen, 
           x= REGION_N)) +
  geom_boxplot() +
